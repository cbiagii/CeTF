...
Then we will associate the genes in both conditions with Gene Ontology (GO). 
GroupGO analysis will be performed where a set of genes are counted within all 
possible ontologies, without statistical power. This function is capable of 
perform groupGO for any organism as long as it's annotation package exists (i.e. 
org.Hs.eg.db, org.Bt.eg.db, org.Rn.eg.db, etc). In this example we will perform 
only for first condition:
