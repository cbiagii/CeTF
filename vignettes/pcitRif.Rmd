...
author: "Carlos Biagi Jr, Ricardo Perecin Nociti, Patrícia Ruy, Marcelo Gomes, João Paulo Bianchi Ximenez, Rafael dos Santos Bezerra, Wilson Araújo da Silva Junior"
...
# Subsetting the data by selecting the genes that have the highest expression values. Is recommended to perform PCIT analysis for each group of conditions.
PCIT_input <- Clean_Dat[rowSums(Clean_Dat)>77, 1:4]

# Performing PCIT analysis
PCIT_out <- PCIT(PCIT_input, tolType = "mean")
kable(PCIT_out[[1]][1:10, ]) %>% kable_styling(bootstrap_options = "striped", full_width = FALSE)

# Adjacency matrix: View(PCIT_out[[2]])
# Adjacency matrix with only the significant values: View(PCIT_out[[3]])
```
...
```{r densitySig, eval=TRUE,fig.height = 4, fig.width = 6, fig.align = "center"}
...
## RIF
To perform only the RIF analysis we will need the count data, an annotation table and a list with the Transcript Factors of specific organismo (Homo sapiens in this case). The output will be a dataframe with the average expression, RIF1 and RIF2 metrics for each Transcript Factor (TF).
```{r RIF, eval=TRUE}
# Loading packages
library(pcitRif)
library(airway)
...
# Subsetting the data by selecting the genes that have the highest expression values.
...
RIF_out <- RIF(input = RIF_input,
               ...
               ncond1 = 4,
               ncond2 = 4)
kable(RIF_out[1:10, ]) %>% kable_styling(bootstrap_options = "striped", full_width = FALSE)
```
