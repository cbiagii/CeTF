...
## RIF
To perform the RIF analysis we will need the count data, an annotation table and a list with the Transcript Factors of specific organism (Homo sapiens in this case). The output will be a dataframe with the average expression, RIF1 and RIF2 metrics for each Transcript Factor (TF).
...
The first output is a network for both conditions resulted from \emph{runAnalysis}.
...
After this we are able to plot the network of previously obtained ontologies. It's possible to choose which variables ("pathways", "TFs" or "genes") we want to group. All these variables are inputted by the user, that is, the user can choose the pathways, TFs or genes returned from analysis from the runAnalysis function and pcitrif class object, or can also choose by specific pathways, TFs or genes of interest.
...
networkPlot(netCond = getNet1(out), 
            netGO = t2, 
            keyTFs = getKeyTF(out))
```
