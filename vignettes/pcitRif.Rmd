...
To access the pcitRif object is needed to use the Accessors from **pcitrif** class.
...
# Accessing the network for condition 1
  genes <- unique(c(as.character(getNetworkCond1(out)[,1]), as.character(getNetworkCond1(out)[,2])))
...
netGOplot(netCond = getNetworkCond1(out), 
          resultsGO = t1, 
          netGO = t2, 
          anno = getAnno(out), 
          label = TRUE)
...
networkPlot(netCond = getNetworkCond1(out), 
            netGO = t2, 
            keyTFs = getRIFkeyTF(out))
```
