...
#'
#' @description Provides a report of number of elements, mean,
#' standard deviation, minimum and maximum for each of two
#' columns and calculate the correlation and regression for
#' the columns.
#'
...
bivar.awk <- function(x) {
    ...
    cat(blue(sprintf("################################" %+% "\n" %+% "Correlation  =  %s", 
        format(round(cor(x[, 1], x[, 2]), 7), nsmall = 7)) %+% "\n" %+% sprintf("Regression   =  %s", 
