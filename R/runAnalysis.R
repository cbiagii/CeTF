...
#'
#' @export
runAnalysis <- function(counts, conditions = NULL, lfc = 2.57, padj = 0.05, TFs = NULL, ncond1 = NULL,
    ncond2 = NULL, tolType = "mean", diffMethod = "Reverter") {
    ...
    cat(green("##### STEP 1: TPM filter #####" %+% "\n"))
    colnames(counts)[seq_len(ncond1)] <- paste0(colnames(counts)[seq_len(ncond1)], "_", conditions[1])
    ...
    ns_ave <- data.frame(sum = apply(tpm.j != 0, 1, sum), mean = as.numeric(ifelse(tmp1 > 0,
        tmp2/tmp1, 0)))
    st <- bivar.awk(ns_ave)
    ...
    if (diffMethod == "Reverter") {
        de.j <- data.frame(cond1 = apply(Clean_Dat[, grep(conditions[1], colnames(Clean_Dat))],
            1, mean), cond2 = apply(Clean_Dat[, grep(conditions[2], colnames(Clean_Dat))], 1,
            mean))
        tmp1 <- de.j[, 1] - de.j[, 2]
        ...
        Target <- expDiff(counts = tmp1, anno = anno, conditions = conditions, lfc = lfc, padj = padj)
    }
    ...
    # Run RIF
    RIF_out <- RIF(input = RIF_input, nta = nrow(DE_unique), ntf = length(TF_unique), ncond1 = ncond1,
        ncond2 = ncond2)
