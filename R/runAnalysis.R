...
#'
#' @export
runAnalysis <- function(mat, conditions = NULL, lfc = 2.57, padj = 0.05, TFs = NULL,
    nSamples1 = NULL, nSamples2 = NULL, tolType = "mean", diffMethod = "Reverter",
    data.type = NULL) {
    ...
    # Count connections for each gene in cond1 and in cond2, focussing on Key
    # TFs
    ...
    anno <- data.frame(genes = genes, class = ifelse(genes %in% KeyTF$TF, "TF",
        "gene"))
