...
histPlot <- function(mat) {
    ...
    pt1 <- ggplot(df1, aes(clustcoef)) + geom_histogram(breaks = seq(0, 0.6, by = 0.05), col = "black", 
        fill = "#1F3552") + ggtitle("Connectivity Distribution") + xlab("Proportion of Connections") + 
        ylab("Number of Genes") + scale_y_continuous(expand = c(0, 0)) + scale_x_continuous(labels = percent) + 
        theme_bw() + theme(axis.line = element_line(size = 1, colour = "black"), panel.grid.major = element_line(colour = "#d3d3d3"), 
    ...
    pt2 <- ggplot(df2, aes(x = clustcoef)) + geom_step(stat = "ecdf", col = "red", size = 1) + 
        scale_y_continuous(labels = percent) + ggtitle("Connectivity Distribution") + xlab("Number of Genes") + 
