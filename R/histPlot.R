...
histPlot <- function(mat) {
    ...
    pt2 <- ggplot(df2, aes(x = clustcoef)) + geom_step(stat = "ecdf", col = "red", 
        size = 1) + scale_y_continuous(labels = percent) + ggtitle("Connectivity Distribution") + 
        xlab("Number of Genes") + ylab("Cumulative Proportion") + theme_bw() + 
