...
PCIT <- function(input, tolType = "mean") {
    ...
    for (i in seq_len(nrow(gene_pcorr) - 2)) {
        ...
        for (j in (i + 1):(nrow(gene_pcorr) - 1)) {
            for (k in (j + 1):nrow(gene_pcorr)) {
                ...
                if (abs(rxy) < abs(rxz * tol) & abs(rxy) <
                  abs(ryz * tol)) {
                  gene_pcorr2[i, j] <- gene_pcorr2[j,
                    i] <- 0
                }
                if (abs(rxz) < abs(rxy * tol) & abs(rxz) <
                  abs(ryz * tol)) {
                  gene_pcorr2[i, k] <- gene_pcorr2[k,
                    i] <- 0
                }
                if (abs(ryz) < abs(rxy * tol) & abs(ryz) <
                  abs(rxz * tol)) {
                  gene_pcorr2[j, k] <- gene_pcorr2[k,
                    j] <- 0
                }
            }
        }
    ...
    gene_corr <- gene_corr[duplicated(t(apply(gene_corr,
        1, sort))), ]
