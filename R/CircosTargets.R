...
CircosTargets <- function(object, file, nomenclature, selection, cond) {
  ...
  genes <- unique(c(as.character(net[["gene1"]][net[which(net[["gene1"]] %in% selection), 2]]), 
                    as.character(net[["gene2"]][net[which(net[["gene2"]] %in% selection), 1]])))
  ...
  if (nomenclature == "SYMBOL") {
    bedOut <- data.frame(Chr = GTF[, 1], Start = GTF[,4], End = GTF[,5], Gene = GTF[["gene_name"]], stringsAsFactors = FALSE)
  } else if (nomenclature == "ENSEMBL") {
    bedOut <- data.frame(Chr = GTF[, 1], Start = GTF[,4], End = GTF[,5], Gene = GTF[["gene_id"]], stringsAsFactors = FALSE)
  } else {
    stop("nomenclature should be SYMBOL or ENSEMBL")
  }
  colnames(bedOut) <- c("chr", "start", "end", "gene")
  
  annotBed <- subset(bedOut, bedOut[["chr"]] %in% c(seq_len(50), "X", "Y"))
  ...
  annotBed[["chr"]] <- factor(annotBed[["chr"]], levels = c(sort(numbers), sort(letters)))
  ...
  if (length(selection) == 1) {
    ...
    circos.initialize(annotBed[["chr"]], x = annotBed[["start"]], xlim = c(0, 1), sector.width = as.numeric(ln[,2]))
    circos.track(annotBed[["chr"]], ylim = c(0, 1), panel.fun = function(x, y) {
    ...
    bed2[["value1"]] <- (bed2[["value1"]]-min(bed2[["value1"]]))/(max(bed2[["value1"]])-min(bed2[["value1"]]))
    bed2 <- bed2[,c(1,2,3,5)]
    ...
    for(i in seq_along(bed1[["chr"]])) {
      if (bed1[["chr"]][i] == bed2[["chr"]][i]) {
        circos.link(bed1[["chr"]][i], bed1[["value1"]][i], 
                    bed2[["chr"]][i], bed2[["value1"]][i], 
                    border = 1, col = "red")
      } else {
        circos.link(bed1[["chr"]][i], bed1[["value1"]][i], 
                    bed2[["chr"]][i], bed2[["value1"]][i], 
                    border = 1, col = "#00000080")
      }
    }
    ...
    for(i in seq_along(selection)) {
      ...
      genes <- c(as.character(subset(net, net[["gene1"]] == selection[i])[,2]), 
                 as.character(subset(net, net[["gene2"]] == selection[i])[,1]))
      ...
      for(j in seq_along(bed1[["chr"]])) {
        if (bed1[["chr"]][j] == bed2[["chr"]][j]) {
          circos.link(bed1[["chr"]][j], bed1[["value1"]][j], 
                      bed2[["chr"]][j], bed2[["value1"]][j], 
                      border = 1, col = "red")
        } else {
          circos.link(bed1[["chr"]][j], bed1[["value1"]][j], 
                      bed2[["chr"]][j], bed2[["value1"]][j], 
