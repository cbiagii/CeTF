...
singleNetworkPlot <- function(x) {
    ...
    mainTFs <- x@step4$keytf[order(x@step4$keytf$freq.diff, decreasing = TRUE), ]
    ...
    pt1 <- ggnet2(nt1, color = "color", color.legend = "", palette = y, edge.size = 0.5, edge.color = "gray70", 
        label.size = 0.5, alpha = 0.75, size = "degree", edge.alpha = 0.5, label = labels, 
        legend.position = "bottom") + coord_equal() + guides(size = FALSE) + ggtitle(paste("Network:", 
        gsub("freq.", "", colnames(x@step4$keytf)[5]))) + theme(panel.background = element_rect(fill = "white", 
        colour = "grey50"))
    ...
    names(y) = levels(x2)
    labels <- c(as.character(head(mainTFs, 2)[, 1]), as.character(tail(mainTFs, 2)[, 1]))
    pt2 <- ggnet2(nt2, color = "color", color.legend = "", palette = y, edge.size = 0.5, edge.color = "gray70", 
        label.size = 0.5, alpha = 0.75, size = "degree", edge.alpha = 0.5, label = labels, 
        legend.position = "bottom") + coord_equal() + guides(size = FALSE) + ggtitle(paste("Network:", 
        gsub("freq.", "", colnames(x@step4$keytf)[6]))) + theme(panel.background = element_rect(fill = "white", 
        colour = "grey50"))
