...
#'
#' @export
SmearPlotTF <- function(object, diffMethod, lfc = 1.5, conditions = NULL, TF,
    label = FALSE) {
    ...
    c1 <- c(as.character(getNet1(object)[getNet1(object)[, 1] %in% TF, 2]),
        as.character(getNet1(object)[getNet1(object)[, 2] %in% TF, 1]))
    c2 <- c(as.character(getNet2(object)[getNet2(object)[, 1] %in% TF, 2]),
        as.character(getNet2(object)[getNet2(object)[, 2] %in% TF, 1]))
    ...
    pt <- ggplot(subset(dt, Type != "TF")) + geom_point(aes(x = log2(baseMean),
        ...
        Type == paste("Target", conditions[1])), aes(x = log2(baseMean), y = subset(dt,
        Type == paste("Target", conditions[1]))[[var1]], colour = paste("Target",
        conditions[1])), size = 2) + geom_point(data = subset(dt, Type == paste("Target",
        conditions[2])), aes(x = log2(baseMean), y = subset(dt, Type == paste("Target",
        conditions[2]))[[var1]], colour = paste("Target", conditions[2])), size = 2) +
        geom_point(data = subset(dt, Type == "TF"), aes(x = log2(baseMean),
            y = subset(dt, Type == "TF")[[var1]], colour = "TF"), size = 2) +
        theme_bw() + ylab(var2) + xlab(var3) + ggtitle(paste0("Smear Plot for ",
        TF, " and its targets")) + theme(legend.position = "top") + labs(colour = "") +
        scale_colour_manual(values = c("gray80", "goldenrod3", "forestgreen",
            "red")) + geom_hline(yintercept = lfc, linetype = "dashed", color = "black") +
    ...
    if (label) {
        ...
        return(pt + geom_label_repel(data = dtsub, aes(x = log2(baseMean), y = dtsub[[var1]],
            color = Type, alpha = 0.3, label = genes), nudge_y = 1, segment.size = 0.2,
