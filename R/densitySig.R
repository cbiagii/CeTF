...
densitySig <- function(mat1, mat2, threshold = 0.5) {
    ...
    df <- rbind(df1, df2)
    pt1 <- ggplot(df, aes(corr)) + geom_histogram(data = subset(df, sig == "All"), 
        aes(fill = sig), breaks = seq(-1, 1, by = 0.05), col = "black") + geom_histogram(data = subset(df, 
        sig == "PCIT Significant"), aes(fill = sig), breaks = seq(-1, 1, by = 0.05), 
        col = "black") + scale_fill_manual(name = "", values = c("gray", "black"), 
        labels = c("All", "PCIT Significant")) + ggtitle("Density Distribution of Correlation Coefficients") + 
    ...
    pt2 <- ggplot(df, aes(corr)) + geom_histogram(data = subset(df, sig == "All"), 
        aes(fill = sig), breaks = seq(-1, 1, by = 0.05), col = "black") + geom_histogram(data = subset(df, 
        sig == paste0("abs. cor. > ", threshold)), aes(fill = sig), breaks = seq(-1, 
        1, by = 0.05), col = "black") + geom_histogram(data = subset(df, sig == 
        "PCIT Significant"), aes(fill = sig), breaks = seq(-1, 1, by = 0.05), 
        col = "black") + scale_fill_manual(name = "", values = c("red", "gray", 
        "black"), labels = c(paste0("abs. cor. > ", threshold), "All", "PCIT Significant")) + 
        xlab("Correlation Coefficient") + ylab("Frequency") + scale_y_continuous(expand = c(0, 
        0)) + theme_bw() + theme(axis.line = element_line(size = 1, colour = "black"), 
        panel.grid.major = element_line(colour = "#d3d3d3"), panel.grid.minor = element_blank(), 
        panel.border = element_blank(), panel.background = element_blank(), 
    ...
    df <- rbind(df1, df2, df3)
    pt3 <- ggplot(df, aes(corr)) + geom_histogram(data = subset(df, sig == "All"), 
        aes(fill = sig), breaks = seq(-1, 1, by = 0.05), col = "black", alpha = 0.8) + 
        geom_histogram(data = subset(df, sig == "PCIT Significant"), aes(fill = sig), 
            breaks = seq(-1, 1, by = 0.05), col = "black", alpha = 0.9) + geom_histogram(data = subset(df, 
        sig == paste0("abs. cor. > ", threshold)), aes(fill = sig), breaks = seq(-1, 
