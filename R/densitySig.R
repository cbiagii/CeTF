...
densitySig <- function(mat1, mat2, threshold=0.5) {
  ...
  pt2 <- ggplot(df,aes(corr))+
    geom_histogram(data=subset(df,sig=='All'),aes(fill=sig), breaks=seq(-1, 1, by = 0.05), col="black") +
    geom_histogram(data=subset(df,sig==paste0("abs. cor. > ", threshold)),aes(fill=sig), breaks=seq(-1, 1, by = 0.05), col="black") +
    geom_histogram(data=subset(df,sig=='PCIT Significant'),aes(fill=sig), breaks=seq(-1, 1, by = 0.05), col="black") +
    scale_fill_manual(name="", values=c("red","gray","black"),labels=c(paste0("abs. cor. > ", threshold), "All", "PCIT Significant")) +
    ggtitle("Density Distribution of Correlation Coefficients") +
    xlab("Correlation Coefficient") + ylab("Frequency") +
    scale_y_continuous(expand = c(0, 0)) +
    theme_bw() +
  ...
  pt3 <- ggplot(df,aes(corr))+
    ...
    geom_histogram(data=subset(df,sig==paste0("abs. cor. > ", threshold)),aes(fill=sig), breaks=seq(-1, 1, by = 0.05), col="black", alpha = 0.8) +
    scale_fill_manual(name="", values=c("gray10","gray","darkred"),labels=c(paste0("abs. cor. > ", threshold), "All", "PCIT Significant")) +
    ggtitle("Density Distribution of Correlation Coefficients") +
    xlab("Correlation Coefficient") + ylab("Frequency") +
    scale_y_continuous(expand = c(0, 0)) +
    theme_bw() +
  ...
  return(grid.arrange(grobs = list(pt1, pt2, pt3),
