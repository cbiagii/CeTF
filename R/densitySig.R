...
densitySig <- function(mat1, mat2, threshold = 0.5) {
    ...
    df2 <- data.frame(corr = mat2[intersect(which(as.single(mat2) != 0), 
        which(upper.tri(mat1)))], sig = "PCIT Significant")
    df3 <- data.frame(corr = mat1[intersect(which(abs(mat1) > threshold), 
        which(upper.tri(mat1)))], sig = paste0("abs. cor. > ", threshold))
    
    df <- rbind(df1, df2)
    pt1 <- ggplot(df, aes(corr)) + geom_histogram(data = subset(df, sig == 
        "All"), aes(fill = sig), breaks = seq(-1, 1, by = 0.05), col = "black") + 
        geom_histogram(data = subset(df, sig == "PCIT Significant"), aes(fill = sig), 
            breaks = seq(-1, 1, by = 0.05), col = "black") + scale_fill_manual(name = "", 
        values = c("gray", "black"), labels = c("All", "PCIT Significant")) + 
    ...
    pt2 <- ggplot(df, aes(corr)) + geom_histogram(data = subset(df, sig == 
        "All"), aes(fill = sig), breaks = seq(-1, 1, by = 0.05), col = "black") + 
        geom_histogram(data = subset(df, sig == paste0("abs. cor. > ", 
            threshold)), aes(fill = sig), breaks = seq(-1, 1, by = 0.05), 
            col = "black") + geom_histogram(data = subset(df, sig == "PCIT Significant"), 
        aes(fill = sig), breaks = seq(-1, 1, by = 0.05), col = "black") + 
        scale_fill_manual(name = "", values = c("red", "gray", "black"), 
            labels = c(paste0("abs. cor. > ", threshold), "All", "PCIT Significant")) + 
    ...
    df <- rbind(df1, df2, df3)
    pt3 <- ggplot(df, aes(corr)) + geom_histogram(data = subset(df, sig == 
        "All"), aes(fill = sig), breaks = seq(-1, 1, by = 0.05), col = "black", 
        alpha = 0.8) + geom_histogram(data = subset(df, sig == "PCIT Significant"), 
        aes(fill = sig), breaks = seq(-1, 1, by = 0.05), col = "black", 
        alpha = 0.9) + geom_histogram(data = subset(df, sig == paste0("abs. cor. > ", 
        threshold)), aes(fill = sig), breaks = seq(-1, 1, by = 0.05), col = "black", 
        alpha = 0.8) + scale_fill_manual(name = "", values = c("gray10", 
        "gray", "darkred"), labels = c(paste0("abs. cor. > ", threshold), 
