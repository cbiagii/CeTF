...
#'
#' @return Returns an list with the network plot with interaction between genes, TFs and/or ontologies and
#' the table of network used to the plot.
#'
...
#' pt <- networkPlot(netCond = getNet1(pcitrifExample),
#'                   netGO = t2,
...
networkPlot <- function(netCond, netGO, keyTFs) {
    if (missing(netCond)) {
        stop("No \"netCond\" parameter provided")
    }
    if (missing(netGO)) {
        stop("No \"netGO\" parameter provided")
    }
    if (missing(keyTFs)) {
        stop("No \"keyTFs\" parameter provided")
    }
    ...
    x <- factor(ifelse(x %in% TFs, "TFs", ifelse(x %in% gns, "Gene", ifelse(x %in% 
        pathways, "GO:BP", ""))))
